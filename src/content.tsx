...
const AI_BTN_OPEN_SVG = `<svg
  ...
  xmlns="http://www.w3.org/2000/svg">
  <path
    d="M13.3334 25.3333L10.0001 18L2.66675 14.6667L10.0001 11.3333L13.3334 4L16.6667 11.3333L24.0001 14.6667L16.6667 18L13.3334 25.3333ZM24.0001 28L22.3334 24.3333L18.6667 22.6667L22.3334 21L24.0001 17.3333L25.6667 21L29.3334 22.6667L25.6667 24.3333L24.0001 28Z"
    fill="#2563EB"
  />
</svg>`
...
const PlasmoOverlay = () => {
  ...
  const addButton = useCallback((parentNode: HTMLElement) => {
    if (!parentNode.querySelector("#ai-btn")) {
      const btn = document.createElement("button")
      btn.innerHTML = AI_BTN_SVG
      btn.style.cssText = `position:absolute; bottom:12%; right:12%;`
      btn.id = "ai-btn"
      btn.onclick = (e) => {
        e.stopPropagation()
        btn.innerHTML = AI_BTN_OPEN_SVG
        setCurrentElement(parentNode)
  ...
  const removeButton = useCallback((parentNode: HTMLElement) => {
    const btn = parentNode.querySelector("#ai-btn")
    if (btn) {
      btn.remove()
    }
  }, [])

  //function to handle DOM mutation when observed by MutationObserver
    ...
    (mutations: MutationRecord[]) => {
      mutations.forEach((mutation) => {
        if (
          ...
          //to find the closest form element with an id that starts with 'msg-form-ember'
  ...
  //function to setup MutationObserver to observe msg-overlay
  ...
  useEffect(() => {
    ...
    //if document is not ready, sets up eventListener. Otherwise calls handleLoad function to setup observer
  ...
  return (
    <div id="my-extension-root">
      <PromptModal
        ...
        onClose={() => setIsDialogBoxHidden(true)}
